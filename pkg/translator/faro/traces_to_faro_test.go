...
func generateScopeSpans(t *testing.T) ptrace.ScopeSpans {
	t.Helper()
	var scopeSpans = ptrace.NewScopeSpans()
...
func TestTranslateTracesToFaroPayload(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			wantPayloads: func() []*faroTypes.Payload {
				payloads := make([]*faroTypes.Payload, 0)
				payloads = append(payloads, internal.PayloadFromFile(t, "payload-traces-meta-app-name-1.json"))
				payloads = append(payloads, internal.PayloadFromFile(t, "payload-traces-meta-app-name-2.json"))
				return payloads
			}(),
			wantErr: assert.NoError,
		},
		{
			...
			wantPayloads: func() []*faroTypes.Payload {
				payloads := make([]*faroTypes.Payload, 0)
				payloads = append(payloads, internal.PayloadFromFile(t, "payload-traces-two-traces-same-resource.json"))
				return payloads
			}(),
			wantErr: assert.NoError,
		},
